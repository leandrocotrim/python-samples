...
def get_data():
    ...
    texts = emails['email'].str.lower().str.split(' ')
    classification = emails['classificacao']

    setEmail = set()
    for t in texts: setEmail.update(t)
...
dumb_value = max(Counter(classification_test))
print('Score of "dumb" Algorithm is {}.'.format( dumb_value / len(classification_test) * 100 ))
...
score = fit_predict(winer_algorithm, vectors_fit, classification_fit, vectors_test, classification_test)
